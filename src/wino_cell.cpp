...
void winoPEB_CENT(
		...
		ap_uint<OUT_WIDTH*BATCH_SIZE> out_buffer_4[WINO_H2][WINO_W2][WINO_OUT_SIZE_CELL][OUTPUT_BUFFER_DEPTH],
		ap_uint<OUT_WIDTH*BATCH_SIZE> out_buffer_5[WINO_H2][WINO_W2][WINO_OUT_SIZE_CELL][OUTPUT_BUFFER_DEPTH],
		#endif
		...
		)
{
	...
	#pragma HLS array_partition variable = out_buffer_0 dim=1 complete
	#pragma HLS array_partition variable = out_buffer_0 dim=2 complete
	#pragma HLS array_partition variable = out_buffer_0 dim=3 complete
	#pragma HLS array_partition variable = out_buffer_1 dim=1 complete
	#pragma HLS array_partition variable = out_buffer_1 dim=2 complete
	#pragma HLS array_partition variable = out_buffer_1 dim=3 complete
	#pragma HLS array_partition variable = out_buffer_2 dim=1 complete
	#pragma HLS array_partition variable = out_buffer_2 dim=2 complete
	#pragma HLS array_partition variable = out_buffer_2 dim=3 complete
	#pragma HLS array_partition variable = out_buffer_3 dim=1 complete
	#pragma HLS array_partition variable = out_buffer_3 dim=2 complete
	#pragma HLS array_partition variable = out_buffer_3 dim=3 complete

    #if WINO_DOMAIN_SIZE == 6 
    #pragma HLS array_partition variable = out_buffer_4 dim=1 complete
	#pragma HLS array_partition variable = out_buffer_4 dim=2 complete
	#pragma HLS array_partition variable = out_buffer_4 dim=3 complete

    #pragma HLS array_partition variable = out_buffer_5 dim=1 complete
	#pragma HLS array_partition variable = out_buffer_5 dim=2 complete
	#pragma HLS array_partition variable = out_buffer_5 dim=3 complete
    #endif
	...
	for(int cycle=0;cycle < loop_wino_cell_bound; cycle++)
	{
		...
		#pragma HLS pipeline
		#pragma HLS dependence variable=out_buffer_0 inter false
		...
		for(int wino_w2=0;wino_w2<WINO_W2;wino_w2++)
		{
			...
			// {
				...
				#if WINO_DOMAIN_SIZE==6
				element_wise_mult_block<0>(UV_MUL_TILE,input_tile_reg,weight_tile_reg);
				#else
				element_wise_mult_block<0>(UV_MUL_TILE,input_tile_reg,weight_tile_reg);
				#endif
					...
					for(int cidx=0;cidx<WINO_OUT_SIZE_CELL;cidx++)
					{
						...
						for(int ridx=0;ridx<WINO_OUT_SIZE_CELL;ridx++)
						{
							...
							for(int bidx=0;bidx<BATCH_SIZE;bidx++)
							{
								...
								#else
								ATA[0][ridx][cidx][bidx]=UV[0][ridx][cidx][bidx];
								ATA[1][ridx][cidx][bidx]=UV[1][ridx][cidx][bidx];
								#endif
							}
				...
				for(int c=0;c<WINO_OUT_SIZE_CELL;c++)
				{
					#pragma HLS unroll
					#if WINO_DOMAIN_SIZE == 6
						ap_uint<OUT_WIDTH*BATCH_SIZE> data0[6];
						#pragma HLS array_partition variable=data0 factor=6
						ap_uint<OUT_WIDTH*BATCH_SIZE> data1[6];
						#pragma HLS array_partition variable=data1 factor=6
						...
						else
						{
							...
							data0[4]=out_buffer_4[0][wino_w2][c][outbuffer_addr_reg];
							data0[5]=out_buffer_5[0][wino_w2][c][outbuffer_addr_reg];
							...
							data1[4]=out_buffer_4[1][wino_w2][c][outbuffer_addr_reg];
							data1[5]=out_buffer_5[1][wino_w2][c][outbuffer_addr_reg];
						...
						(out_value[0][4][c][1],out_value[0][4][c][0])=data0[4];
						(out_value[0][5][c][1],out_value[0][5][c][0])=data0[5];
						...
						(out_value[1][4][c][1],out_value[1][4][c][0])=data1[4];
						(out_value[1][5][c][1],out_value[1][5][c][0])=data1[5];
					#else
				...
				// {
					...
					// 	for(int c=0;c<WINO_OUT_SIZE_CELL;c++)
					// 	{
					// 		printf("[%8d %8d]",(int) out_value_back[0][r][c][0]
					// 		,(int) out_value_back[0][r][c][1] 
					// 		);
					// 	}
					// 	printf("\n");
					// }
					...
					// 	for(int c=0;c<WINO_OUT_SIZE_CELL;c++)
					// 	{
					// 		printf("[%8d %8d]",(int) out_value_back[1][r][c][0],
					// 		(int) out_value_back[1][r][c][1] 
					// 		);
					// 	}
					// 	printf("\n");
					// }
				...
				if(loop_omini_base_cnt<=weightbuffer_outdepth_minitile_number)
				{
					...
					for(int c=0;c<WINO_OUT_SIZE_CELL;c++)
					{
						...
						out_buffer_4[0][wino_w2][c][outbuffer_addr_reg]=(out_value_back[0][4][c][1],out_value_back[0][4][c][0]);
						out_buffer_5[0][wino_w2][c][outbuffer_addr_reg]=(out_value_back[0][5][c][1],out_value_back[0][5][c][0]);
						...
						out_buffer_4[1][wino_w2][c][outbuffer_addr_reg]=(out_value_back[1][4][c][1],out_value_back[1][4][c][0]);
						out_buffer_5[1][wino_w2][c][outbuffer_addr_reg]=(out_value_back[1][5][c][1],out_value_back[1][5][c][0]);
					...
					for(int cidx=0;cidx<WINO_OUT_SIZE_CELL;cidx++)
					{
						...
						for(int ridx=0;ridx<WINO_OUT_SIZE_CELL;ridx++)
						{
							...
							for(int bidx=0;bidx<BATCH_SIZE;bidx++)
							{
								...
								ATA[1][ridx][cidx][bidx]=UV[1][ridx][cidx][bidx]*4;
								#else
								ATA[0][ridx][cidx][bidx]=UV[0][ridx][cidx][bidx]>>ATA_QUANT_BIT;
								ATA[1][ridx][cidx][bidx]=UV[1][ridx][cidx][bidx]>>ATA_QUANT_BIT;
								#endif
							}
